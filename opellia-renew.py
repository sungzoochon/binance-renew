...
while True:
 try:    
  for i in LIST:
   ...
   if (-240 <= CCI(I) <= -210) and (cur_price < BOLL(I)[2]) and ((K[199] + D[199])/2 < 15) and (I not in LONG_BUY_LIST) and (len(LONG_BUY_LIST) + len(SHORT_BUY_LIST) < 5) and I not in Long_candidate_list:
    ticker = binance.fetch_ticker(I)
    cur_price = ticker['last']
    Long_candidate_list.append(I)
    Long_Max.append(cur_price)
 
   if (240 >= CCI(I) >= 220) and (cur_price > BOLL(I)[0]) and ((K[199] + D[199])/2 > 85) and (I not in SHORT_BUY_LIST) and (len(SHORT_BUY_LIST) + len(LONG_BUY_LIST) < 5) and  I not in Short_candidate_list and now.hour != 9:
    ticker = binance.fetch_ticker(I)
    cur_price = ticker['last']
